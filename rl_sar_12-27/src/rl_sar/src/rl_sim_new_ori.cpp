    ...
    if (this->params.observations_history.size() != 0)
    {
        ...
        {   
            ...
            if(this->t == 5)
            {
                ...
                this->camera_input = torch::cat({second, newest}, /*dim=*/1); // [1,2,58,87]

                // 传入模型的 camera_input 为类成员
                torch::Tensor camera_input = this->camera_input;


                // inputs.push_back(obs_p_history);
                ...
                inputs.clear();
                // ---- 取最后 32 维 ----
                // auto last_32 = depth_latent_and_yaw.index(
                //     {torch::indexing::Slice(), torch::indexing::Slice(-32, torch::indexing::None)}
                // );  // [1, 32]

                // // ---- 新增：拼大 tensor ----
                // if (first_) {
                //     latent_acc_ = last_32.clone();   // [1, 32]
                //     first_ = false;
                // } else {
                //     latent_acc_ = torch::cat({latent_acc_, last_32}, 0);  // 沿0维拼，变成[N, 32]
                // }

                // // ---- 保存 ----
                // torch::Tensor latent = latent_acc_.cpu();   // [N, 32]
                // torch::save(latent, "latents/z_tm_batch.pt");
            }
            else
            {
