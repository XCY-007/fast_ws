...
torch::Tensor RL_Sim::Forward()
{
    ...
    if (this->params.observations_history.size() != 0)
    {
        ...
        else if (this->mode == 0)
        {
            // mode parkour
            // ---------------相机
            torch::Tensor depth_camera = this->depth.unsqueeze(0);
            // ---------------相机 end

            // obs_student[:, 6:8] = 0
            obs_p.index({torch::indexing::Slice(), torch::indexing::Slice(6, 8)}).zero_();
            ...
            if(this->t == 5)
            {
                inputs.push_back(depth_camera);
                inputs.push_back(obs_p);
            ...
            int n_cols = depth_latent_and_yaw.size(1);
            torch::Tensor depth_latent = depth_latent_and_yaw.index({torch::indexing::Slice(), torch::indexing::Slice(0, n_cols - 2)});

            // yaw = depth_latent_and_yaw[:, -2:]
            //torch::Tensor yaw = depth_latent_and_yaw.index({torch::indexing::Slice(), torch::indexing::Slice(-2, None)});
            ...
            //std::cout << "obs_p: " << obs_p << std::endl;
            // 假设 obs_p shape [1, N]
            torch::Tensor zeros_170 = torch::zeros({1, 170}, obs_p.options());
            torch::Tensor obs_parkour = torch::cat({obs_p, zeros_170, obs_p_history}, -1);

            //std::cout << "obs_parkour: " << obs_parkour.defined() << ", shape: " << obs_parkour.sizes() << std::endl;
            //std::cout << "depth_latent: " << depth_latent.defined() << ", shape: " << depth_latent.sizes() << std::endl;


            // actions_parkour = demo_go2.policy(obs_parkour, hist_encoding=True, scandots_latent=depth_latent)
            inputs.push_back(obs_parkour);
            inputs.push_back(depth_latent);
