...
torch::Tensor RL_Sim::Forward()
{
    ...
    if (this->params.observations_history.size() != 0)
    {
        ...
        //std::cout << "history_obs total size: " << this->history_obs.sizes() << std::endl;

        // obs_p: [B, 53]
        torch::Tensor obs_p = current_obs.slice(1, 0, 53);
        //std::cout << "obs_p: " << obs_p << std::endl;

        // obs_r: [B, 42]
        torch::Tensor obs_r = current_obs.slice(1, 53, 95);

        int n_history = 950 / 95;

        // 变形 [B, T, 95]
        torch::Tensor history_obs_flatten = history_obs.view({-1, n_history, 95});

        // [B, 53*T]
        torch::Tensor obs_p_history = history_obs_flatten.slice(2, 0, 53)
        ...
        obs_p.index({torch::indexing::Slice(), 10}) = this->obs.commands.index({torch::indexing::Slice(), 0});

        obs_p.index({torch::indexing::Slice(), 6}) = this->obs.commands.index({torch::indexing::Slice(), 2});
        obs_p.index({torch::indexing::Slice(), 7}) = this->obs.commands.index({torch::indexing::Slice(), 2});
        ...
        torch::Tensor parkour_val = obs_p.index({torch::indexing::Slice(), 10});
        ...
        if (switch_to_recovery) {
            ...
            this->mode = 0;
            this->camera_model.run_method("reset");
        }
        ...
        {   
            ...
            torch::Tensor depth_latent_and_yaw_op;
            torch::Tensor depth_latent_and_yaw_vp;
            torch::Tensor loss;
            if(this->t == 5)
            {
                ...
                {
                    // --- 测试逻辑：将输入强制置零 ---
                    //torch::Tensor zero_obs_history = torch::zeros_like(obs_p_history);
                    //torch::Tensor zero_camera_input = torch::zeros_like(this->camera_input);
                    
                    // 构造全零输入向量
                    //std::vector<torch::jit::IValue> test_inputs = {zero_obs_history, zero_camera_input};
                    // ------------------------------

                    inputs = {obs_p_history, camera_input};
                    std::vector<torch::jit::IValue> inputs_obs = {obs_p_history};
                    std::vector<torch::jit::IValue> inputs_camera = {camera_input};
                    loss = this->selector.forward(inputs_camera).toTensor();
                    //std::cout << std::setprecision(6) << "selector loss: " << loss.item<float>() << std::endl;
                    //this->camera_model.run_method("reset");
                    depth_latent_and_yaw_vp = this->camera_model.forward(inputs).toTensor();
                    //std::cout << "Zero Input VP Output:\n" << depth_latent_and_yaw_vp << std::endl;

                    depth_latent_and_yaw_op = this->op_estimator.forward(inputs_obs).toTensor();
                    depth_latent_and_yaw = torch::cat({depth_latent_and_yaw_vp * 1, depth_latent_and_yaw_op * 0}, /*dim=*/1);
                }
