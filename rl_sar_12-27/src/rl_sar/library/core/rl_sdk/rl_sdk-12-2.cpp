...
torch::Tensor RL::ComputeObservation()
{
    ...
    //std::cout << "Parkour OBS-GET-start" << std::endl;
    //std::cout << "commands shape: " << this->obs.commands.sizes() << std::endl;
    // ang_vel
    obs_list.push_back(this->obs.ang_vel * this->params.ang_vel_scale); // 3  
    // 计算RPY
    //torch::Tensor rpy = this->quatToRPY(this->obs.base_quat);
    //std::cout << "rpy shape: " << rpy.sizes() << std::endl;
    // torch::Tensor r_p = this->quatToRPY(this->obs.base_quat).index({torch::indexing::Slice(), torch::indexing::Slice(0,2)});
    // obs_list.push_back(r_p); // 2
    obs_list.push_back(this->QuatRotateInverse(this->obs.base_quat, this->obs.gravity_vec)); // 3
    // commands 0
    obs_list.push_back(torch::zeros({1,1}, this->obs.ang_vel.options())); // 1
    obs_list.push_back(torch::zeros({1,1}, this->obs.ang_vel.options())); // 1
    obs_list.push_back(torch::zeros({1,1}, this->obs.ang_vel.options())); // 1
    // obs_list.push_back(this->obs.commands.index({torch::indexing::Slice(), 0}).unsqueeze(-1)); // 1
    // obs_list.push_back(this->obs.commands.index({torch::indexing::Slice(), 1}).unsqueeze(-1)); // 1
    // delta_yaw
    // obs_list.push_back(this->obs.commands.index({torch::indexing::Slice(), 2}).unsqueeze(-1)); // 1 
    // delta_next_yaw
    //obs_list.push_back(torch::zeros({1,1}, this->obs.ang_vel.options())); // 1
    // joint_pos
    ...
    obs_list.push_back(dof_pos_rel * this->params.dof_pos_scale); // 12
    // joint_vel
    obs_list.push_back(this->obs.dof_vel * this->params.dof_vel_scale); // 12
    // last actions
    obs_list.push_back(this->obs.actions); // 12
    // foot contact
    //obs_list.push_back(this->obs.foot_contact); // 4
    //std::cout << "Parkour OBS-GET-end" << std::endl;
...
void RL::InitObservations()
{
    ...
    this->obs.foot_contact = torch::tensor({{0.0, 0.0, 0.0, 0.0}});
...
void RL::InitRL(std::string robot_path)
{
    ...
    this->model = torch::jit::load("/home/kemove/quadruped_robot/rl_sar/src/rl_sar/policy/go2/robot_lab/policy.pt");
    this->extra_model = torch::jit::load("/home/kemove/quadruped_robot/rl_sar/src/rl_sar/policy/go2/robot_lab/policy-delay-1.pt");
    this->camera_model = torch::jit::load("/home/kemove/quadruped_robot/rl_sar/src/rl_sar/policy/go2/robot_lab/depth_latest.pt");
}
...
// void RL::ComputeOutput(const torch::Tensor &actions,
...
//     // PD 控制
//     torch::Tensor error_pos = output_dof_pos - this->obs.dof_pos;
//     torch::Tensor error_vel = output_dof_vel - this->obs.dof_vel;

//     output_dof_tau = this->params.rl_kp * error_pos + this->params.rl_kd * error_vel;
...
// 输入: q [N,4], 顺序是 (w, x, y, z)
// 输出: rpy [N,3], 顺序是 (roll, pitch, yaw)
// 输入: q [N,4], 顺序 (w, x, y, z)
// 输出: rpy [N,3], 顺序 (roll, pitch, yaw)，单位: 弧度，范围 [-π, π]
torch::Tensor RL::quatToRPY(torch::Tensor q)
{
    q = q.to(torch::kFloat32);
    q = q / q.norm(2, -1, true);  // 归一化
    ...
    // roll
    auto sinr_cosp = 2.0 * (w * x + y * z);
    auto cosr_cosp = 1.0 - 2.0 * (x * x + y * y);
    auto roll = torch::atan2(sinr_cosp, cosr_cosp);

    // pitch
    auto sinp = 2.0 * (w * y - z * x);
    auto pitch = torch::asin(torch::clamp(sinp, -1.0, 1.0));

    // yaw
    auto siny_cosp = 2.0 * (w * z + x * y);
    auto cosy_cosp = 1.0 - 2.0 * (y * y + z * z);
    auto yaw = torch::atan2(siny_cosp, cosy_cosp);

    // 拼回 [N,3]
    auto rpy = torch::stack({roll, pitch, yaw}, -1);

    // wrap 到 [-π, π]，直接用 atan2(sin, cos)
    rpy = torch::atan2(torch::sin(rpy), torch::cos(rpy));
...
// warp2pi: 将输入角度限制在 [-pi, pi] 范围
// torch::Tensor RL::warp2pi(const torch::Tensor& angle) {
//     // 确保输入是浮点类型
//     torch::Tensor angle_f = angle.to(torch::kFloat32);

//     // ((angle + pi) % (2*pi)) - pi
//     torch::Tensor two_pi = 2 * M_PI * torch::ones_like(angle_f);
//     torch::Tensor wrapped = torch::remainder(angle_f + M_PI, two_pi) - M_PI;
