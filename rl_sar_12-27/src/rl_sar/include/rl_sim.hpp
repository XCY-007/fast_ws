...
private:
    ...
    int t = 4;  // 计数器
    int mode = 1;
    torch::Tensor prev_obs_input;  // 保存上一次 obs_input
    torch::Tensor last_depth_latent_and_yaw; // 保存上一次的 depth_latent_and_yaw
    torch::Tensor camera_input; // shape [1,2,58,87] 双帧深度缓存
    torch::Tensor latent_acc_;   // 攒所有 latent
    bool first_ = true;          // 第一次 flag
    ...
    gazebo_msgs::msg::ContactsState FLfoot_force;
    gazebo_msgs::msg::ContactsState FRfoot_force;
    gazebo_msgs::msg::ContactsState RLfoot_force;
    gazebo_msgs::msg::ContactsState RRfoot_force;

    std::array<std::atomic<double>, 4> foot_force_z_;
    
    rclcpp::Subscription<gazebo_msgs::msg::ContactsState>::SharedPtr FLfoot_subscriber;
    rclcpp::Subscription<gazebo_msgs::msg::ContactsState>::SharedPtr FRfoot_subscriber;
    rclcpp::Subscription<gazebo_msgs::msg::ContactsState>::SharedPtr RLfoot_subscriber;
    rclcpp::Subscription<gazebo_msgs::msg::ContactsState>::SharedPtr RRfoot_subscriber;
    ...
    void GazeboFootCallback(const gazebo_msgs::msg::ContactsState::SharedPtr msg, int foot_number);
    void DepthCallback(const sensor_msgs::msg::Image::SharedPtr msg);

    void saveObsToCSV(const torch::Tensor& obs, const std::string& filename);
#endif
